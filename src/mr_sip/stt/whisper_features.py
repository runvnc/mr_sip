...
This avoids importing transformers/torch in the Smart Turn endpointing path and
keeps mr_sip aligned with Pipecat's current LocalSmartTurnAnalyzerV3 behavior.
"""
...
def compute_whisper_log_mel_features(audio: np.ndarray, *, do_normalize: bool = True) -> np.ndarray:
    ...
    Args:
        audio: 1-D float audio at 16 kHz. Caller may provide exactly 8 seconds
            left-padded per Smart Turn docs; if not, this function right-pads or
            truncates to 8 seconds to mirror WhisperFeatureExtractor behavior.
        do_normalize: Apply waveform zero-mean/unit-variance normalization before
            spectrogram calculation.

    Returns:
        float32 ndarray shaped (80, 800).
    """
    if audio.ndim != 1:
