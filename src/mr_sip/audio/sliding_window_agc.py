...
RED_BG_WHITE_TEXT = '\x1b[41m\x1b[97m'
RESET_COLOR = '\x1b[0m'

class SlidingWindowAGC:
    """Smooth AGC using sliding window RMS and exponential gain smoothing."""

    def __init__(self, target_rms: float=0.15, max_gain: float=4.0, min_gain: float=0.5, window_seconds: float=1.5, smoothing: float=0.95, sample_rate: int=16000):
        """
        ...
        logger.info(f'SlidingWindowAGC initialized: target_rms={target_rms:.3f}, max_gain={max_gain:.1f}x, window={window_seconds:.1f}s, smoothing={smoothing:.2f}')
    ...
    def process_chunk(self, audio_chunk: np.ndarray) -> np.ndarray:
        ...
        self.current_gain = self.smoothing * self.current_gain + (1.0 - self.smoothing) * desired_gain
        ...
        if self.chunks_processed % 100 == 0:
            ...
            logger.debug(f'AGC stats: current_gain={self.current_gain:.3f}, window_rms={window_rms:.4f}, avg_gain_change={avg_change:.4f}')
    ...
    def get_stats(self) -> dict:
        """Get AGC statistics."""
        avg_change = self.total_gain_changes / self.chunks_processed if self.chunks_processed > 0 else 0.0
        return {'current_gain': self.current_gain, 'target_rms': self.target_rms, 'max_gain': self.max_gain, 'window_seconds': self.window_seconds, 'smoothing': self.smoothing, 'chunks_processed': self.chunks_processed, 'avg_gain_change_per_chunk': avg_change, 'buffer_fill': len(self.audio_buffer) / self.window_size}