...
class MindRootSIPBot(BareSIP):
    ...
    def handle_call_established(self):
        ...
        time.sleep(0.5)  # Give baresip time to create ports
        ...
        # Removed fixed delay - _find_current_audio_files now polls
    ...
    def _find_current_audio_files(self):
        ...
        # Poll for audio files instead of fixed delay
    ...
    def _process_incoming_audio_realtime(self):
        ...
        try:
            # Poll for WAV header to be written (wait for file to have data)
            max_attempts = 20  # 2 seconds max
            attempt = 0
            while attempt < max_attempts:
                if os.path.exists(self.current_dec_file):
                    file_size = os.path.getsize(self.current_dec_file)
                    if file_size > 100:  # Has header + some data
                        logger.info(f"WAV file ready after {attempt * 0.1:.1f}s (size: {file_size} bytes)")
                        break
                time.sleep(0.1)
                attempt += 1
            
            if attempt >= max_attempts:
                logger.warning("Timeout waiting for WAV file to be ready")
