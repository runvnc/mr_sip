...
class MindRootSIPBot(BareSIP):
    ...
    def __init__(self, user, password, gateway, audio_dir='.', on_utterance_callback=None, model_size='small', context=None):
        """
        Args:
            user: SIP username
    ...
    async def _on_utterance(self, text, utterance_num, timestamp):
        ...
        utterance_data = {'number': utterance_num, 'text': text, 'timestamp': timestamp, 'time_str': time.strftime('%H:%M:%S', time.localtime(timestamp))}
    ...
    def handle_call_established(self):
        ...
        if WhisperStreamingVAD:
            ...
            self.transcriber = WhisperStreamingVAD(model_size=self.model_size, sample_rate=16000, chunk_duration=0.5, silence_threshold=0.01, silence_duration=1.0, min_speech_duration=0.5)
        ...
        if self.current_dec_file:
            ...
            self.audio_processor_thread = threading.Thread(target=self._process_incoming_audio_realtime, daemon=True)
            ...
            while self.transcriber.is_running:
                try:
                    ...
                    if msg_type == 'utterance':
                        result = self.transcriber.model.transcribe(audio_chunk, language=None, fp16=False, verbose=False)
                        text = result['text'].strip()
                        if text and text != self.transcriber.last_transcription:
                            self.transcriber.utterance_count += 1
                            timestamp = time.time()
                            if asyncio.get_event_loop().is_running():
                                asyncio.create_task(self._on_utterance(text, self.transcriber.utterance_count, timestamp))
    ...
    def _process_incoming_audio_realtime(self):
        ...
        wait_count = 0
        while (not os.path.exists(self.current_dec_file) or os.path.getsize(self.current_dec_file) < 100) and wait_count < max_wait_attempts:
            time.sleep(0.1)
            wait_count += 1
        ...
        try:
            ...
            with open(self.current_dec_file, 'rb') as f:
                f.seek(data_offset)
                chunk_size = 8000 * sample_width * channels
                empty_read_count = 0
                max_empty_reads = 100
                logger.info('Starting real-time transcription...')
